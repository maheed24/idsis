    ...
    <script>
        ...
        function loadCertficateByDetailsId(response) {
            ...
            certificate_license.forEach(function(certificate_license) {
                ...
                if (certificate_license.cert_type_id === 1) {
                    cell1.textContent = 'CERTIFICATE OF OWNERSHIP';
                }
                if (certificate_license.cert_type_id === 2) {
                    cell1.textContent = 'CERTIFICATE OF PHILIPPINE REGISTRY';
                }
                if (certificate_license.cert_type_id === 3) {
                    cell1.textContent = 'BAY AND RIVER LICENSE';
                }
                if (certificate_license.cert_type_id === 4) {
                    cell1.textContent = 'COASTWISE LICENSE';
                }
                if (certificate_license.cert_type_id === 5) {
                    cell1.textContent = 'CERTIFICATE OF OWNERSHIP RECREATIONAL BOAT';
                }
                if (certificate_license.cert_type_id === 6) {
                    cell1.textContent = 'RECREATIONAL BOAT CERTIFICATE';
                }
                if (certificate_license.cert_type_id === 7) {
                    cell1.textContent = 'FISHING VESSEL SAFETY CERTIFICATE';
                }
                if (certificate_license.cert_type_id === 8) {
                    cell1.textContent = 'CARGO SAFETY VESSEL';
                }
                if (certificate_license.cert_type_id === 9) {
                    cell1.textContent = 'PERMIT TO OPERATE';
                }
                if (certificate_license.cert_type_id === 10) {
                    cell1.textContent = 'TONNAGE MEASUREMENT CERTIFICATE';
                }
                ...
                cell3.textContent = certificate_license.sec_no;

                let cell4 = document.createElement('td');
                cell4.textContent = certificate_license.date_issued;

                let cell5 = document.createElement('td');
                cell5.textContent = certificate_license.validity;

                let cell6 = document.createElement('td');
                cell6.textContent = certificate_license.or_no;

                let cell7 = document.createElement('td');
                cell7.textContent = certificate_license.or_date;
                        ...
                        $.ajax({
                            ...
                            success: function(response) {
                                if (response.status == 404) {
                                    ...
                                    $('#cert_type_id1').select2().val(certificate_license
                                            .cert_type_id)
                                        .trigger('change');
                ...
                link1.id = 'link1' + certificate_license.cert_id; // Example: Unique ID for each link

                // Add style and class to the link
                link1.style.background = '#367fa9'; // Set background color
                link1.className = 'button btn btn-success btn-sm editPayment btn-flat'; // Add CSS classes
                ...
                // Attach the icon to the link
                link1.appendChild(icon1);

                // Append the link to cell9
                cell9.appendChild(link1);
        ...
        function updateShipPropulsion(edittableRow, data) {
            $.ajax({
                ...
                error: function(err) {
                    $('#update_co_cpr_msg').html(
                        "<div class='alert alert-danger alert-dismissible'><button type='button' class='close' data-dismiss='alert' aria-hidden='true'>&times;</button><b>" +
                        err + '</b> </div>');
                }
            });
        }
